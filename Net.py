...
class DiceBCELoss(nn.Module):
    ...
    def forward(self, inputs, targets, smooth=1e-4):
        weight = torch.where(targets == 1, torch.ones_like(targets) * 4, torch.ones_like(targets) / 5)
        bce = torch.nn.BCEWithLogitsLoss(weight=weight)
        inputs = torch.sigmoid(inputs)
        BCE = bce(inputs, targets)
        inputs = inputs.view(-1)
        targets = targets.view(-1)
        intersection = (inputs * targets).sum()
        dice_loss = 1 - (2 * intersection) / (inputs.sum() + targets.sum() + smooth)
